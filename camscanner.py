...
# image = cv2.imread("xx.webp")
# image = cv2.imread("bubblesheet.jpg")
# image = cv2.imread("trial-2.jpg")
...
def transformation(image):
    ...
    if np.std(image) < 20 or (height > 1600 and width > 1600):
        return image
    ...
    if approx_contour is None:
        return image  # Skip transformation if no contour found
...
def scan(image):
    cropped_image = transformation(image)
    cleaned_image = final_image(cropped_image)
